...
void	init_shared_args(t_shared *shared_args, int ac, char **av)
{
	...
	shared_args->free_queue = NULL;
	shared_args->dump = NULL;
	shared_args->tail = NULL;
	pwrap(pthread_mutex_init(&shared_args->queue_mutex, NULL), 4);
	pwrap(pthread_mutex_init(&shared_args->dump_mutex, NULL), 4);
	pwrap(pthread_mutex_init(&shared_args->flag_mutex, NULL), 4);
	if (ac >= 6)
